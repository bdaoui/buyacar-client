...
console.log(chosenMessage[0])
  ...
  return (
    <div className='flex flex-col text-gold w-full text-base md:text-lg lg:text-xl p-10'>
        ...
        <h1>Message posted by: <span className='text-white'>{chosenMessage[0].name} {chosenMessage[0].lastName}</span></h1>
        </header>

        <section className='flex flex-row border-b-2 border-gold justify-evenly p-4'>
  
            <p>Téléphoner: <span className='text-white'>{chosenMessage[0].phone}</span></p>
            <p>Email: <span className='text-white'>{chosenMessage[0].email}</span></p>
