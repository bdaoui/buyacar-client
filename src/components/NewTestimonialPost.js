...
const NewTestimonialPost = ({ handleCloseSecondSection, refresh, setRefresh }) => {
  ...
  return (
    <div className='p-10 flex flex-col w-full h-screen'>
        ...
        <input className="p-5" type="text" onChange={e => setAuthor(e.target.value)} name="author" />
        <label for="body" className='text-gold text-lg my-5' >Message</label>
        <textarea className='h-full md:h-80 p-5' type="text" onChange={e => setBody(e.target.value)} name="body"/>
